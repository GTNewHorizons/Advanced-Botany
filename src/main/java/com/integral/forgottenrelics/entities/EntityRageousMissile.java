...
/**
 * Advanced implementation of EntityMagicMissile from Botania. This stuff definitely makes some work.
 * 
 * @author Integral
 */
...
public class EntityRageousMissile extends EntityThrowable {
    ...
    public EntityLivingBase getTargetEntity() {
        ...
        if (e != null && e instanceof EntityLivingBase) return (EntityLivingBase) e;
    ...
    public void onUpdate() {
        ...
        if (target != null) {
            ...
            motionX = motionVec.x;
            motionY = motionVec.y;
            if (time < 30) motionY = Math.abs(motionY);
            motionZ = motionVec.z;

            /*
             * if(time < 20) { //motionY = Math.abs(motionY); Vector3 randomtVec = new Vector3(this.posX+((Math.random()
             * - 0.5D)*16), this.posY+((Math.random() - 0.5D)*16), this.posZ+((Math.random() - 0.5D)*16)); Vector3
             * diffRandomVec = randomtVec.copy().sub(thisVec); Vector3 motionRandomVec =
             * diffRandomVec.copy().normalize().multiply(evil ? 0.5 : 0.6); motionX = motionRandomVec.x; motionY =
             * motionRandomVec.y; motionZ = motionRandomVec.z; }
             */
            ...
            if (evil && diffVec.mag() < 1) setDead();
        } else {

            Vector3 targetVec = new Vector3(
                    this.posX + ((Math.random() - 0.5D) * 16),
                    this.posY + ((Math.random() - 0.5D) * 16),
                    this.posZ + ((Math.random() - 0.5D) * 16));
            Vector3 diffVec = targetVec.copy().sub(thisVec);
            Vector3 motionVec = diffVec.copy().normalize().multiply(0.5);
            motionX = motionVec.x;
            motionY = motionVec.y;
    ...
    public boolean getTarget() {
        ...
        double range = 32;
        entities = worldObj.getEntitiesWithinAABBExcludingEntity(
                this.getTrueThrower(),
                AxisAlignedBB.getBoundingBox(
                        posX - range,
                        posY - range,
        ...
        if (pos.entityHit != null & getTargetEntity() == pos.entityHit) {
            ...
            if (thrower != null) {
                ...
                pos.entityHit.attackEntityFrom(
                        DamageSource.causeIndirectMagicDamage(this, thrower),
                        (float) (RelicsConfigHandler.nuclearFuryDamageMIN
                                + (Math.random() * (RelicsConfigHandler.nuclearFuryDamageMAX
                                        - RelicsConfigHandler.nuclearFuryDamageMIN))));
            }
