...
public class EntityCrimsonOrb extends EntityThrowable implements IEntityAdditionalSpawnData {
    ...
    public EntityCrimsonOrb(final World par1World, final EntityLivingBase c, final EntityLivingBase t,
            final boolean r) {
        super(par1World, c);
        this.targetID = 0;
        this.casterID = 0;
    ...
    protected void onImpact(final MovingObjectPosition mop) {
        if (!this.worldObj.isRemote && this.getThrower() != null
            ...
            mop.entityHit.attackEntityFrom(
                    DamageSource.causeIndirectMagicDamage((Entity) this, (Entity) this.getThrower()),
                    (float) (RelicsConfigHandler.crimsonSpellDamageMIN
                            + (Math.random() * (RelicsConfigHandler.crimsonSpellDamageMAX
                                    - RelicsConfigHandler.crimsonSpellDamageMIN))));
    ...
    public void getNewTarget() {
        ...
        List<EntityLivingBase> entities = this.worldObj.getEntitiesWithinAABB(
                ...
                AxisAlignedBB.getBoundingBox(
                        this.posX - searchRange,
                        this.posY - searchRange,
                        this.posZ - searchRange,
                        this.posX + searchRange,
                        this.posY + searchRange,
                        this.posZ + searchRange));
        ...
        if (entities.size() > 0) this.target = entities.get((int) ((entities.size() - 1) * Math.random()));
    }
    ...
    public void onUpdate() {
        ...
        if (this.target != null) {
            ...
            if (this.ticksExisted < 5 & this.motionY < 0.0D) this.motionY = Math.abs(this.motionY);
        } else {
            this.getNewTarget();
        }
    }
