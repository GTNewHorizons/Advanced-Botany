...
public class ItemLunarFlares extends Item implements IWarpingGear {
    ...
    public void addInformation(ItemStack par1ItemStack, EntityPlayer par2EntityPlayer, List par3List, boolean par4) {
        if (GuiScreen.isShiftKeyDown()) {
            ...
            par3List.add(
                    StatCollector.translateToLocal("item.ItemLunarFlares3_1.lore") + " "
                            + (int) RelicsConfigHandler.damageLunarFlareImpact
                            + " "
                            + StatCollector.translateToLocal("item.ItemLunarFlares3_2.lore"));
            par3List.add(StatCollector.translateToLocal("item.ItemLunarFlares35.lore"));
            par3List.add(StatCollector.translateToLocal("item.FREmpty.lore"));
            par3List.add(
                    StatCollector.translateToLocal("item.ItemLunarFlares4_1.lore") + " "
                            + (int) RelicsConfigHandler.damageLunarFlareDirect
                            + " "
                            + StatCollector.translateToLocal("item.ItemLunarFlares4_2.lore"));
    ...
    public void spawnLunarFlare(World world, EntityPlayer player, MovingObjectPosition mop) {
        ...
        if (mop != null & !world.isRemote) {
            ...
            flare.setPosition(
                    mop.blockX + ((Math.random() - 0.5D) * 12),
                    mop.blockY + 24 + ((Math.random() - 0.5D) * 12),
                    mop.blockZ + 0.5 + ((Math.random() - 0.5D) * 12));

            Vector3 posVec = new Vector3(mop.blockX + 0.5, mop.blockY, mop.blockZ + 0.5);
            Vector3 motVec = new Vector3(
                    (Math.random() - 0.5) * 18,
                    (24 + (Math.random() - 0.5) * 18) * 2.0D,
                    (Math.random() - 0.5) * 18);
            posVec.add(motVec);
    ...
    public void onUsingTick(ItemStack stack, EntityPlayer player, int count) {
        if (count != getMaxItemUseDuration(stack) && count % 2 == 0 && !player.worldObj.isRemote) {
            ...
            if (mop != null) if (WandManager.consumeVisFromInventory(
                    player,
                    new AspectList().add(Aspect.AIR, this.AerCost).add(Aspect.FIRE, this.IgnisCost)
                            .add(Aspect.ORDER, this.OrdoCost))) {
                                this.spawnLunarFlare(player.worldObj, player, mop);
                                if (count != getMaxItemUseDuration(stack) && count % 4 == 0
                                        && !player.worldObj.isRemote)
                                    player.worldObj.playSoundEffect(
                                            player.posX,
                                            player.posY,
                                            player.posZ,
                                            "ForgottenRelics:sound.starfall",
                                            2.0F,
                                            (float) (1.0F + (Math.random() * 0.5F)));
                            }
