...
/**
 * This code is heavily based on such of Rod of Shaded Mesa from Botania, and is generally an ideological inheritor of
 * it. It's our sacred duty to make telekinesis GREAT AGAIN!
 * 
 * @author Integral
 */
...
public class ItemTelekinesisTome extends Item implements IWarpingGear {
    ...
    public void addInformation(ItemStack par1ItemStack, EntityPlayer par2EntityPlayer, List par3List, boolean par4) {
        if (GuiScreen.isShiftKeyDown()) {
            ...
            par3List.add(StatCollector.translateToLocal("item.FREmpty.lore"));
            par3List.add(
                    StatCollector.translateToLocal("item.ItemTelekinesisTome11_1.lore") + " "
                            + (int) RelicsConfigHandler.telekinesisTomeDamageMIN
                            + "-"
                            + (int) RelicsConfigHandler.telekinesisTomeDamageMAX
                            + " "
                            + StatCollector.translateToLocal("item.ItemTelekinesisTome11_2.lore"));
            ...
            par3List.add(
                    " " + StatCollector.translateToLocal("item.FRAerCost.lore")
                            + ItemChaosTome.round(((this.AerCost / 100.0D) / 3.0D), 2));
            par3List.add(
                    " " + StatCollector.translateToLocal("item.FRPerditioCost.lore")
                            + ItemChaosTome.round(((this.OrdoCost / 100.0D) / 3.0D), 2));
            par3List.add(StatCollector.translateToLocal("item.FREmpty.lore"));
            par3List.add(StatCollector.translateToLocal("item.FRVisPerCast.lore"));
            par3List.add(
                    " " + StatCollector.translateToLocal("item.FRAerCost.lore")
                            + (80 / 100.0D) * RelicsConfigHandler.telekinesisTomeVisMult);
            par3List.add(
                    " " + StatCollector.translateToLocal("item.FROrdoCost.lore")
                            + (50 / 100.0D) * RelicsConfigHandler.telekinesisTomeVisMult);
            par3List.add(
                    " " + StatCollector.translateToLocal("item.FRIgnisCost.lore")
                            + (200 / 100.0D) * RelicsConfigHandler.telekinesisTomeVisMult);
        } else {
    ...
    public ItemStack onItemRightClick(ItemStack stack, World world, EntityPlayer player) {
        ...
        if (ticksCooldown == 0) {
            ...
            if (item != null & re_dist == -1) re_dist = player.getDistanceToEntity(item);

            if (item != null) {

                if (WandManager.consumeVisFromInventory(
                        player,
                        new AspectList().add(Aspect.AIR, this.AerCost).add(Aspect.ORDER, this.OrdoCost))) {
                    ...
                    for (int i = 0; i <= 6; i++) {
                        ...
                        Botania.proxy
                                .wispFX(world, entityCenter.x, entityCenter.y, entityCenter.z, r, g, b, s, xm, ym, zm);
                    }
                    ...
                    if (SuperpositionHandler.isEntityBlacklistedFromTelekinesis(item)) return stack;
    ...
    public void leftClick(EntityPlayer player) {
        ...
        if (stack != null && stack.getItem() == Main.itemTelekinesisTome) {
            ...
            if (targetID != -1 && player.worldObj.getEntityByID(targetID) != null) {
                ...
                if (item != null) this.lightningAttack(player, item, stack, player.worldObj);
            }
        }
    }
    ...
    /**
     * Attempts to find an existing target for Telekinesis Tome by given entity ID. This basically allows to smoothly
     * control any entity once it was targeted, without Tome constantly switching to other targets within scan zone.
     * 
     * @param player
     * @param world
     * @param targetID
    ...
    public EntityLivingBase getExistingTarget(EntityPlayer player, World world, int targetID, float range) {
        ...
        while (entities.size() == 0 && distance < 25) {
            ...
            if (entities.contains(taritem)) return taritem;
        }
    ...
    public static void lightningAttack(EntityPlayer player, EntityLivingBase target, ItemStack stack, World world) {
        ...
        if (world.isRemote || ItemNBTHelper.getInt(stack, "AttackCooldown", 0) != 0) return;

        Vector3 TVec = Vector3.fromEntityCenter(target);
        Vector3 moveVector = new Vector3(player.getLookVec().normalize());

        if (player.getDistanceToEntity(target) <= 16) if (WandManager.consumeVisFromInventory(
                player,
                new AspectList().add(Aspect.AIR, (int) (80 * RelicsConfigHandler.telekinesisTomeVisMult))
                        .add(Aspect.ORDER, (int) (50 * RelicsConfigHandler.telekinesisTomeVisMult))
                        .add(Aspect.FIRE, (int) (200 * RelicsConfigHandler.telekinesisTomeVisMult)))) {

                            for (int counter = 0; counter <= 3; counter++) SuperpositionHandler.imposeLightning(
                                    player.worldObj,
                                    player.dimension,
                                    player.posX,
                                    player.posY + 1.0,
                                    ...
                                    TVec.z,
                                    20,
                                    (float) ((1 / player.getDistanceToEntity(target))
                                            * (2.0F + (Math.random()) * 4.0F)),
                                    (int) (player.getDistanceToEntity(target) * 1.2F),
                                    0,
                                    (float) (0.225F + (player.getDistanceSqToEntity(target) / 2000)));
                            ...
                            target.attackEntityFrom(
                                    ...
                                    ((float) (16 + (24 * Math.random())) * 1.0F));

                        }

        if (player.isSneaking()) if (WandManager.consumeVisFromInventory(
                player,
                new AspectList().add(Aspect.AIR, (int) (150 * RelicsConfigHandler.telekinesisTomeVisMult))
                        .add(Aspect.ORDER, (int) (80 * RelicsConfigHandler.telekinesisTomeVisMult)))
