...
public class EntitySeed extends EntityThrowable {
	...
	public void onUpdate() {
		...
		if(this.worldObj.isRemote) {
			...
			for(int i = 0; i < 5; i++) {
				...
				Botania.proxy.wispFX(worldObj, posX, posY, posZ, color.getRed() / 255.0f, color.getGreen() / 255.0f, color.getBlue() / 255.0f, (float) (0.0625f * size + (Math.random() * 0.12f)), mx, my, mz, 0.5f);	
			}
		}
	}
			...
			if(seed != null && seed.getItem() instanceof ItemGrassSeeds) {
				...
				for(int i = 0; i < getRadius(); i++) {
					for(int k = 0; k < getRadius(); k++) {
						...
						int j = posY;
						if(isTopBlock(posX, posY - 1, posZ))
							j = Math.max(0, j - 20);
						for(; !isTopBlock(posX, j, posZ); j++) {
							if(Math.abs(j - posY) > 40)
								break;	
						}
						posY = j;
	...
	private void spawnGrowParticle(int posX, int posY, int posZ) {
		for (int i = 0; i < 50; i++) {
			...
			float velMul = 0.025F;
			Color color = ItemSprawlRod.getSeedColor(getSeed());
	        Botania.proxy.wispFX(worldObj, posX + 0.5D + x, posY + 0.5D + y, posZ + 0.5D + z, color.getRed() / 255.0f, color.getGreen() / 255.0f, color.getBlue() / 255.0f, (float)Math.random() * 0.15F + 0.15F, (float)-x * velMul, (float)-y * velMul, (float)-z * velMul);
		}
	}
	
	private boolean isDirt(int posX, int posY, int posZ) {
		return worldObj.getBlock(posX, posY, posZ) == Blocks.dirt || worldObj.getBlock(posX, posY, posZ) == Blocks.grass;
	}

	private boolean isTopBlock(int posX, int posY, int posZ) {
		return worldObj.getBlock(posX, posY + 1, posZ).getMaterial() == Material.air || worldObj.getBlock(posX, posY + 1, posZ) instanceof BlockBush;
	}
}