...
public class TileGameBoard extends TileMod implements IRenderHud {
	...
	protected void updateServer() {
		...
		if(worldObj.getTotalWorldTime() % 20 == 0 && hasFullDice() && endGameTick == -1)
			endGameTick = 28;
	...
	public void renderHud(Minecraft mc, ScaledResolution res) {
		...
		RenderItem.getInstance().renderItemAndEffectIntoGUI(mc.fontRenderer, mc.renderEngine, isCustomGame ? (customStack == null ? headRender : customStack) : headRender, x - (!isSingleGame ? 1 : 0), y - (!isSingleGame ? 1 : 0));
		if(!isSingleGame)
