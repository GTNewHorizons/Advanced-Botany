...
public class SubTileAspecolus extends SubTileFunctional {
	...
	public void onUpdate() {
		...
		List<EntityPlayer> players = world.getEntitiesWithinAABB(EntityPlayer.class, AxisAlignedBB.getBoundingBox(posX, posY, posZ, (posX + 1), (posY + 1), (posZ + 1)));		
		int countPlayer = 0;
		for(EntityPlayer p : players) {
			if(!(this.mana >= this.manaRequare))
				break;
			...
			for(Aspect a : aspList.getAspects()) {
				if(!Aspect.getPrimalAspects().contains(a) && Thaumcraft.proxy.playerKnowledge.getAspectPoolFor(p.getCommandSenderName(), a) > 0)
					aspects.add(a);
			}
			if(aspects.isEmpty())
				continue;
			for(int i = 0; i < 8; i++) {
				...
				if(!(Thaumcraft.proxy.playerKnowledge.getAspectPoolFor(p.getCommandSenderName(), a) > 0))
					continue;
				...
				for(int r = 0; r < components.length; r++) {
					...
					PacketHandler.INSTANCE.sendTo(new PacketAspectPool(components[r].getTag(), (short) 1, Thaumcraft.proxy.playerKnowledge.getAspectPoolFor(p.getCommandSenderName(), components[r])),
							(EntityPlayerMP) p);
				}
			}
