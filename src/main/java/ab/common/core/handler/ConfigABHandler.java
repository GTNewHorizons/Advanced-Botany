...
public class ConfigABHandler {
	...
	public static String[] relicNames = new String[] { "infiniteFruit", "kingKey", "flugelEye", "thorRing", "odinRing", "lokiRing",
			"freyrSlingshot", "keyHiddenRiches", "pocketWardrobe", "sphereNavigation", "hornPlenty" };
	...
	public static void load() {
		...
		maxContainerMana[0] = loadPropInt(CATEGORY_BALANCE, "Mana Container Capacity", desc, false, maxContainerMana[0]);
		desc = "Maximum mana capacity for Diluted Mana Container";
		maxContainerMana[1] = loadPropInt(CATEGORY_BALANCE, "Mana Container Capacity (Diluted)", desc, false, maxContainerMana[1]);
		desc = "Maximum mana capacity for Fabulous Mana Container";
		maxContainerMana[2] = loadPropInt(CATEGORY_BALANCE, "Mana Container Capacity (Fabulous)", desc, false, maxContainerMana[2]);
		desc = "Protection factor for nebula armour";
		protectionFactorNebula = loadPropDouble(CATEGORY_BALANCE, "Nebula Armor Protection Factor", desc, false, protectionFactorNebula);
		desc = "Damage factor for Space Blade";
		damageFactorSpaceSword = loadPropDouble(CATEGORY_BALANCE, "Space Sword Damage Factor", desc, false, damageFactorSpaceSword);
		desc = "The number of ticks needed to restore 1 unit of the Rod of Nebula strength.";
		nebulaWandCooldownTick = loadPropInt(CATEGORY_BALANCE, "Rod of Nebula Cooldown", desc, false, nebulaWandCooldownTick);
		...
		lockWorldNameNebulaRod = loadPropString(CATEGORY_BALANCE, "Locking Worlds for Teleportation with Nebula Rod", desc, false, lockWorldNameNebulaRod);
		desc = "Limitation on X Z coordinates for teleportation, do not increase the default value";
		limitXZCoords = loadPropInt(CATEGORY_BALANCE, "Restriction on X Z coordinates for Rod of Nebula", desc, false, limitXZCoords);
		...
		nebulaRodManaCost = loadPropInt(CATEGORY_BALANCE_MANA_COST, "Rod of Nebula repair Mana cost", desc, false, nebulaRodManaCost);
		
		desc = "Mana cost to try to find the specified block";
		sphereNavigationManaCost = loadPropInt(CATEGORY_BALANCE_MANA_COST, "Sphere of Navigation Mana cost", desc, false, sphereNavigationManaCost);
		
		//Relics Settings
		
		lockEntityListToHorn = loadPropString(CATEGORY_RELICS, "Blocked creatures for double drop", desc, false, lockEntityListToHorn);
		...
		for(int i = 0; i < fateBoardRelicEnables.length; i++)
			fateBoardRelicEnables[i] = loadPropBool(CATEGORY_RELICS, "Enable relic: " + relicNames[i], desc, false, fateBoardRelicEnables[i]);
		
		//Misc Settings
		
		desc = "Activating the charging animation for the Mana Charger";
		useManaChargerAnimation = loadPropBool("general", "Mana Charger lighting", desc, false, useManaChargerAnimation);		
