...
public class CommonHelper {
    ...
    public static void fertilizer(World world, Block block, int xCoord, int yCoord, int zCoord, int count,
        ...
        if (world.isRemote || !(block instanceof IGrowable)
                ...
                || world.getTileEntity(xCoord, yCoord, zCoord) != null)
            return;
        ...
        if (igrowable.func_149851_a(world, xCoord, yCoord, zCoord, world.isRemote))
            if (igrowable.func_149852_a(world, world.rand, xCoord, yCoord, zCoord)) {
                ...
                for (int i = 0; i < count; i++) {
                    if (meta != world.getBlockMetadata(xCoord, yCoord, zCoord)) return;
                    igrowable.func_149853_b(world, world.rand, xCoord, yCoord, zCoord);
                }
            }
    }

    public static boolean setBlock(World world, Block block, int meta, int x, int y, int z, EntityPlayer player,
            boolean checkAir) {
        if (!world.isRemote) {
            if (checkAir && !(world.getBlock(x, y, z).getMaterial() == Material.air)) return false;
            BlockEvent.BreakEvent event = new BlockEvent.BreakEvent(
                    x,
                    y,
                    z,
    ...
    private static boolean setBlockWithY(World world, Block block, int meta, int x, int y, int z) {
        if (y < 256) world.setBlock(x, y, z, block, meta, 3);
        else return false;
        return true;
    }
}
