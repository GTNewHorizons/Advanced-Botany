...
public class ModelManaCharger extends ModelBase {
	...
	public void render(RenderTileManaCharger render, double time) {
		...
		for (int i = 1; i < 5; i++) {
			switch(i) {
			case 1: chargerPlate.rotateAngleY = 3.1416F;
				break;
			case 2: chargerPlate.rotateAngleY = 0;
				break;
			case 3: chargerPlate.rotateAngleY = 1.5708F;
				break;
			case 4: chargerPlate.rotateAngleY = -1.5708F;
				break;
			}
			...
			if(stack != null) {
				...
				if(manaPercent < 100.0f) {
					...
					if(tile.clientTick[i] > 12) {
						...
						float posZ = 0;
						switch(i) {
						case 1: posX = 0.0f; posZ = -0.375f;
							break;
						case 2: posX = 0.0f; posZ = 0.375f;
							break;
						case 3: posX = -0.375f; posZ = 0.0f;
							break;
						case 4: posX = 0.375f; posZ = 0.0f;
							break;
						}
						Vector3 itemVec = Vector3.fromTileEntity(render.charger).add(0.5D + posX + ((Math.random() / 8) - 0.0625f), 0.67D + offset1, 0.5D + posZ + ((Math.random() / 8) - 0.0625f));
						Vector3 tileVec = Vector3.fromTileEntity(render.charger).add(0.5D + posX, 0.7425D + offset1 - (chargeY / 2), 0.5D + posZ);
						LightningHandler.spawnLightningBolt(render.charger.getWorldObj(), itemVec, tileVec, 10.0F, render.charger.getWorldObj().rand.nextLong(), 1140881820, 1140901631);
