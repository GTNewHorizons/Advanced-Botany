...
public class RenderBlockNidavellirForge implements ISimpleBlockRenderingHandler {

    public void renderInventoryBlock(Block block, int metadata, int modelID, RenderBlocks renderer) {
        GL11.glPushMatrix();
        GL11.glTranslatef(-0.5F, -0.375F, -0.5F);
        GL11.glScalef(1.25f, 1.25f, 1.25f);
        TileNidavellirForge forge = new TileNidavellirForge();
        RenderTileManaContainer.metadata = metadata;
        TileEntityRendererDispatcher.instance.renderTileEntityAt((TileEntity) forge, 0.0D, 0.0D, 0.0D, 0.0F);
        GL11.glPopMatrix();
    }
