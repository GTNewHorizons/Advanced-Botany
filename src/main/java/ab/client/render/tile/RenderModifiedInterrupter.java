...
public class RenderModifiedInterrupter extends TileEntitySpecialRenderer {
	
	@Override
	public void renderTileEntityAt(TileEntity tile, double x, double y, double z, float fq) {
		if(!((TileCorporeaInterrupter)tile).isModified() || ((TileCorporeaInterrupter)tile).getCatalystTile() == null)
			return;
		Minecraft.getMinecraft().renderEngine.bindTexture(TextureMap.locationBlocksTexture);
		int meta = ((TileCorporeaInterrupter)tile).getCatalystTile().getBlockMetadata();
		...
		TileRenderer.renderLateralSides(x + 0.5, y + 0.5, z + 0.5, 0.504, BlockListAB.blockCorporeaInterrupter.icons[2], t);
