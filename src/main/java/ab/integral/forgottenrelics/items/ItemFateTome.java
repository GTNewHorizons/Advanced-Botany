...
public class ItemFateTome extends Item implements IWarpingGear {
    ...
    public void addInformation(ItemStack par1ItemStack, EntityPlayer par2EntityPlayer, List par3List, boolean par4) {
        ...
        if (par1ItemStack.hasTagCompound()) if (ItemNBTHelper.verifyExistance(par1ItemStack, "IFateCooldown"))
            if (ItemNBTHelper.getInt(par1ItemStack, "IFateCooldown", 0) > 0) {
                par3List.add(StatCollector.translateToLocal("item.FREmpty.lore"));
                par3List.add(
                        StatCollector.translateToLocal("item.FRCode" + (int) ((Math.random() * 15) + 1) + ".lore")
                                + StatCollector.translateToLocal("item.ItemFateTomeCooldown.lore")
                                + " "
                                + new BigDecimal((ItemNBTHelper.getInt(par1ItemStack, "IFateCooldown", 0) / 20.0D))
                                        .setScale(1, BigDecimal.ROUND_HALF_UP).doubleValue()
                                + " "
                                + StatCollector.translateToLocal("item.FRSeconds.lore"));
            }
        par3List.add(StatCollector.translateToLocal("item.FREmpty.lore"));
    }
    ...
    public void onUpdate(ItemStack itemstack, World world, Entity entity, int i, boolean b) {
            ...
            if (ItemNBTHelper.verifyExistance(itemstack, "IFateCooldown")) {
                if (ItemNBTHelper.getInt(itemstack, "IFateCooldown", 0) > 0) {
                    ItemNBTHelper.setInt(
                            itemstack,
                            "IFateCooldown",
                            ItemNBTHelper.getInt(itemstack, "IFateCooldown", 0) - 1);
        ...
        if ((Math.random() <= 0.000016) & (!world.isRemote)) if (entity instanceof EntityPlayer) {
