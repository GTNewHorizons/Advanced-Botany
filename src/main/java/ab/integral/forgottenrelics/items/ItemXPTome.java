...
public class ItemXPTome extends Item {
    ...
    public void addInformation(ItemStack par1ItemStack, EntityPlayer par2EntityPlayer, List par3List, boolean par4) {
        ...
        else cMode = StatCollector.translateToLocal("item.ItemXPTomeExtraction.lore");
        ...
        par3List.add(
                StatCollector.translateToLocal("item.FRCode6.lore") + ItemNBTHelper.getInt(par1ItemStack, "XPStored", 0)
                        + " "
                        + StatCollector.translateToLocal("item.ItemXPTomeUnits.lore")
                        + " "
                        + ExperienceHelper.getLevelForExperience(ItemNBTHelper.getInt(par1ItemStack, "XPStored", 0))
                        + " "
                        + StatCollector.translateToLocal("item.ItemXPTomeLevels.lore"));
    ...
    public void onUpdate(ItemStack itemstack, World world, Entity entity, int i, boolean b) {
        ...
        if (!(entity instanceof EntityPlayer) || world.isRemote
                || !ItemNBTHelper.getBoolean(itemstack, "IsActive", false))
            return;
        ...
        if (ItemNBTHelper.getBoolean(itemstack, this.TAG_ABSORPTION, true)) {
            ...
            if (ExperienceHelper.getPlayerXP(player) >= this.xpPortion) {
                ...
                ItemNBTHelper
                        .setInt(itemstack, "XPStored", ItemNBTHelper.getInt(itemstack, "XPStored", 0) + this.xpPortion);
                action = true;
            } else
                if (ExperienceHelper.getPlayerXP(player) > 0 & ExperienceHelper.getPlayerXP(player) < this.xpPortion) {
        ...
        if (action) player.inventoryContainer.detectAndSendChanges();
