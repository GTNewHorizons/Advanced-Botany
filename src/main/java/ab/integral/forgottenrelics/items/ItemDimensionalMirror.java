...
public class ItemDimensionalMirror extends Item {
    ...
    public void addInformation(ItemStack par1ItemStack, EntityPlayer par2EntityPlayer, List par3List, boolean par4) {
        ...
        if (par1ItemStack.hasTagCompound()) {
            ...
            par3List.add(StatCollector.translateToLocal("item.FREmpty.lore"));
            par3List.add(
                    StatCollector.translateToLocal("item.MirrorX.lore")
                            + ItemNBTHelper.getInt(par1ItemStack, "IStoredX", 0));
            par3List.add(
                    StatCollector.translateToLocal("item.MirrorY.lore")
                            + ItemNBTHelper.getInt(par1ItemStack, "IStoredY", 0));
            par3List.add(
                    StatCollector.translateToLocal("item.MirrorZ.lore")
                            + ItemNBTHelper.getInt(par1ItemStack, "IStoredZ", 0));
            par3List.add(StatCollector.translateToLocal("item.FREmpty.lore"));
            par3List.add(
                    StatCollector.translateToLocal("item.MirrorDimension.lore")
                            + ItemNBTHelper.getInt(par1ItemStack, "IDimensionID", 0));
        }
    }
    ...
    public void onUsingTick(ItemStack stack, EntityPlayer player, int count) {
        ...
        for (int counter = 0; counter <= 3; counter++) player.worldObj.spawnParticle(
                "portal",
                vec.x,
                vec.y,
                vec.z,
        ...
        if (count == 1) {
            ...
            if (!player.worldObj.isRemote & player.dimension != dimension) {
                ((EntityPlayerMP) player).mcServer.getConfigurationManager()
                        .transferPlayerToDimension((EntityPlayerMP) player, dimension);
            }
            ...
            for (int counter = 0; counter <= 128; counter++) player.worldObj.spawnParticle(
                    "portal",
                    player.posX,
                    player.posY - 1.0D,
                    player.posZ,
    ...
    public ItemStack onItemRightClick(ItemStack stack, World world, EntityPlayer player) {
        ...
        if (stack.hasTagCompound()) written = true;
        else written = false;

        if (written & !player.isSneaking()) {

            if (!RelicsConfigHandler.interdimensionalMirror
                    & player.dimension != ItemNBTHelper.getInt(stack, "IDimensionID", 0))
                return stack;
