...
public class ItemAncientAegis extends ItemBaubleBaseModifier {
    ...
    public void addInformation(ItemStack par1ItemStack, EntityPlayer par2EntityPlayer, List par3List, boolean par4) {
        if (GuiScreen.isShiftKeyDown()) {
            par3List.add(
                    StatCollector.translateToLocal("item.ItemAncientAegis1_1.lore") + " "
                            + (int) (RelicsConfigHandler.ancientAegisDamageReduction * 100F)
                            + StatCollector.translateToLocal("item.ItemAncientAegis1_2.lore"));
    ...
    void fillModifiers(Multimap<String, AttributeModifier> attributes, ItemStack stack) {
        attributes.put(
                ...
                new AttributeModifier(getBaubleUUID(stack), "Bauble modifier", 1, 0));
    }
