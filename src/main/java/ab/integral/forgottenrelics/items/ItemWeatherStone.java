...
public class ItemWeatherStone extends Item {
    ...
    public void addInformation(ItemStack par1ItemStack, EntityPlayer par2EntityPlayer, List par3List, boolean par4) {
        if (GuiScreen.isShiftKeyDown()) {
            ...
            par3List.add(
                    StatCollector.translateToLocal("item.ItemWeatherStone2_1.lore") + " "
                            + (int) (this.VisCost / 100)
                            + " "
                            + StatCollector.translateToLocal("item.ItemWeatherStone2_2.lore"));
    ...
    public ItemStack onItemRightClick(ItemStack stack, World world, EntityPlayer player) {
        if (world.isRaining()) player.setItemInUse(stack, stack.getMaxItemUseDuration());
    ...
    public void onUsingTick(ItemStack stack, EntityPlayer player, int count) {
        ...
        if (count == 1 & player.worldObj.getWorldInfo().isRaining()) if (WandManager.consumeVisFromInventory(
                player,
                new AspectList().add(Aspect.AIR, this.VisCost).add(Aspect.EARTH, this.VisCost)
                        .add(Aspect.WATER, this.VisCost))) {
