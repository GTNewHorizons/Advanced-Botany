...
public class ItemObeliskDrainer extends Item implements IWarpingGear, IRepairable {
    ...
    public void onUsingTick(ItemStack stack, EntityPlayer player, int count) {
        ...
        if (!(player.worldObj.getTileEntity((int) x, (int) y, (int) z) instanceof TileEldritchObelisk)
                || player.getDistance(x, y + 2.5, z) > 16) {
        ...
        if (count % 30 == 0 & count != this.getMaxItemUseDuration(stack) & !player.worldObj.isRemote) {
            ...
            for (int counter = 0; counter <= 3; counter++) SuperpositionHandler.imposeLightning(
                    ...
                    20,
                    curve,
                    (int) (player.getDistance(x, y + 2.5, z) * 1.6F),
                    0,
                    0.075F);
            ...
            ItemStack randomWand = SuperpositionHandler
                    .getRandomValidWand(SuperpositionHandler.wandSearch(player), randomAspect);

            if (randomWand != null) ((ItemWandCasting) randomWand.getItem()).addVis(
                    randomWand,
                    randomAspect,
                    (int) ((5 + (Math.random() * 15)) * RelicsConfigHandler.obeliskDrainerVisMult),
                    true);
        ...
        if (!player.worldObj.isRemote & Math.random() <= 0.175) {
            float h = 0.0F;
            if (Math.random() > 0.5) h = 0.4F;
            else h = -0.4F;
            // Main.proxy.lightning(player.worldObj, x+0.5, y+3.0, z+0.5, x+(Math.random() - 0.5D)*4, y+(Math.random() -
            // 0.5D)*4, z+(Math.random() - 0.5D)*4, 20, 0.5F, 10, 0, 0.075F);
            SuperpositionHandler.imposeArcLightning(
                    player.worldObj,
                    player.dimension,
                    x + 0.5,
                    y + 2.5 + ((Math.random() - 0.5D) * 2),
                    z + 0.5,
                    x + 0.5 + ((Math.random() - 0.5D) * 4),
                    y + 2.5 + ((Math.random() - 0.5D) * 4),
                    z + 0.5 + ((Math.random() - 0.5D) * 4),
                    1F,
                    0.6F,
                    1F,
                    h);
        ...
        if (player != null & player.worldObj.isRemote) {
            ...
            Main.proxy.wispFX4(
                    player.worldObj,
                    x + 0.5,
                    (double) (y + 1 + player.worldObj.rand.nextFloat() * 3.0f),
                    z + 0.5,
                    player,
                    5,
                    true,
                    1.0f);
            for (int counterrr = 0; counterrr <= 4; counterrr++) player.worldObj.spawnParticle(
                    "portal",
                    x + 0.5,
                    y + 2.5 + ((Math.random() - 0.5D) * 2),
                    z + 0.5,
                    ((Math.random() - 0.5D) * 3),
                    ((Math.random() - 0.5D) * 0.3),
                    ((Math.random() - 0.5D) * 3));
    ...
    public ItemStack onItemRightClick(ItemStack stack, World world, EntityPlayer player) {
        ...
        for (int counter = 0; counter <= loadedTiles.size() - 1; counter++) {
            ...
            if (player.getDistance(checkedObelisk.xCoord, checkedObelisk.yCoord, checkedObelisk.zCoord) > 16.0D
                    || checkedObelisk.getWorldObj().provider.dimensionId != player.dimension) {
                loadedTiles.remove(counter);
                counter = -1;
                continue;
            } else {
