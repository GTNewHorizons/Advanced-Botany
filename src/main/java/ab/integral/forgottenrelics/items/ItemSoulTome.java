...
public class ItemSoulTome extends Item implements IWarpingGear, IRepairable {
    ...
    public void addInformation(ItemStack par1ItemStack, EntityPlayer par2EntityPlayer, List par3List, boolean par4) {
        if (GuiScreen.isShiftKeyDown()) {
            ...
            par3List.add(
                    " " + StatCollector.translateToLocal("item.FRTerraCost.lore") + (this.TerraCost / 100.0D) * 10.0D);
            par3List.add(
    ...
    public void onUsingTick(ItemStack stack, EntityPlayer player, int count) {
        ...
        List<EntityLivingBase> entities = player.worldObj.getEntitiesWithinAABB(
                ...
                AxisAlignedBB.getBoundingBox(
                        player.posX - searchRange,
                        player.posY - searchRange,
                        player.posZ - searchRange,
                        player.posX + searchRange,
                        player.posY + searchRange,
                        player.posZ + searchRange));
        ...
        if (count < (getMaxItemUseDuration(stack) - 20))
            for (int counter = entities.size() - 1; counter >= 0; counter--) {
                if (player.getDistanceToEntity(entities.get(counter)) <= 3) if (WandManager.consumeVisFromInventory(
                        player,
                        new AspectList().add(Aspect.FIRE, (int) (150 * RelicsConfigHandler.soulTomeVisMult))
                                .add(Aspect.ENTROPY, (int) (120 * RelicsConfigHandler.soulTomeVisMult)))) {
                                    ...
                                    Vector3 diff = (entityVec.copy().sub(playerVec))
                                            .multiply((1 / player.getDistanceToEntity(thrownEntity)) * 3.0F);

                                    float curve = (float) ((1
                                            / player.getDistance(entityVec.x, entityVec.y, entityVec.z)) * 8.0F);

                                    if (!player.worldObj.isRemote)
                                        for (int counterZ = 0; counterZ <= 3; counterZ++) Main.proxy.lightning(
                                                player.worldObj,
                                                player.posX,
                                                player.posY + 1.0,
                                                player.posZ,
                                                ...
                                                40,
                                                curve,
                                                (int) (player.getDistance(entityVec.x, entityVec.y, entityVec.z)
                                                        * 6.0F),
                                                0,
                                                0.075F);
        ...
        /*
         * int range = 4; for(int i = 0; i < 360; i += 16) { //float r = (float) (0.9F + (Math.random() * 0.1F));
         * //float g = (float) (0.1F + (Math.random() * 0.2F)); //float b = 0.0F; float r = -1.0F; float g = 0.8F +
         * (float) Math.random() * 0.2F; float b = 0.4F + (float) Math.random() * 0.6F; float m = 0.15F; float mv =
         * 0.35F; float rad = i * (float) Math.PI / 180F; double x = player.posX - Math.cos(rad) * range; double y =
         * player.posY; double z = player.posZ - Math.sin(rad) * range; Thaumcraft.proxy.blockRunes(player.worldObj,
         * x-0.5, y, z-0.5, r, g, b, 20, (float) ((Math.random() - 0.5D)*0.1)); //Botania.proxy.wispFX(player.worldObj,
         * x, y, z, r, g, b, 0.5F, (float) (Math.random() - 0.5F) * m, (float) (Math.random() - 0.5F) * mv, (float)
         * (Math.random() - 0.5F) * m); }
         */

        if (count < (getMaxItemUseDuration(stack) - 20) & count % 4 == 0) if (WandManager.consumeVisFromInventory(
                player,
                new AspectList().add(Aspect.EARTH, this.TerraCost).add(Aspect.AIR, this.AerCost)
                        .add(Aspect.FIRE, this.IgnisCost).add(Aspect.ENTROPY, this.PerditioCost))) {
                            ...
                            if (randomEntity != null & !player.worldObj.isRemote) {
                                ...
                                if (soulDamage > 20) soulDamage = 20.0F;
                                else if (soulDamage < 1) soulDamage = 1.0F;

                                randomEntity.attackEntityFrom(
                                        new DamageRegistryHandler.DamageSourceSoulDrain(player),
                                        soulDamage);
