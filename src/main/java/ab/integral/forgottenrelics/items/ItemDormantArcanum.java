...
public class ItemDormantArcanum extends ItemBaubleBase implements IBauble {
    ...
    public void addInformation(ItemStack par1ItemStack, EntityPlayer par2EntityPlayer, List par3List, boolean par4) {
        ...
        if (par1ItemStack.hasTagCompound()) {
            ...
            par3List.add(
                    StatCollector.translateToLocal("item.FRCode6.lore")
                            + ((ItemNBTHelper.getInt(par1ItemStack, "ILifetime", 0) * 2) / 100.0D)
                            + StatCollector.translateToLocal("item.ItemDormantArcanum2.lore"));
        ...
        if (itemstack.hasTagCompound() & !entity.worldObj.isRemote & entity instanceof EntityPlayer) {
            ...
            if (ItemNBTHelper.getInt(itemstack, "ILifetime", 0) > 0) {
                ...
                if (WandManager.consumeVisFromInventory(
                        (EntityPlayer) entity,
                        new AspectList().add(Aspect.AIR, 3).add(Aspect.FIRE, 3).add(Aspect.EARTH, 3)
                                .add(Aspect.WATER, 3).add(Aspect.ORDER, 3).add(Aspect.ENTROPY, 3))) {
