...
public class ItemApotheosis extends Item implements IWarpingGear {
    ...
    public void addInformation(ItemStack par1ItemStack, EntityPlayer par2EntityPlayer, List par3List, boolean par4) {
        if (GuiScreen.isShiftKeyDown()) {
            ...
            par3List.add(StatCollector.translateToLocal("item.FREmpty.lore"));
            par3List.add(
                    StatCollector.translateToLocal("item.ItemApotheosis4_1.lore") + " "
                            + (int) RelicsConfigHandler.damageApotheosisDirect
                            + " "
                            + StatCollector.translateToLocal("item.ItemApotheosis4_2.lore"));
            par3List.add(
                    StatCollector.translateToLocal("item.ItemApotheosis5_1.lore") + " "
                            + (int) RelicsConfigHandler.damageApotheosisImpact
                            + " "
                            + StatCollector.translateToLocal("item.ItemApotheosis5_2.lore"));
    ...
    public void spawnBabylonWeapon(EntityPlayer player) {
        if (!player.worldObj.isRemote) {
            ...
            if (rawYaw < 0) yaw = Math.abs(rawYaw);
            else yaw = 360 - rawYaw;
            ...
            if (multV2 < 45) {
                // DO NOTHING
            } else if (multV2 > 45) multV2 = 45 - (multV2 - 45);
            ...
            for (int supercounter = 0; supercounter <= 100; supercounter++) {
                ...
                if (Math.random() >= 0.5) negative = -1.0D;
                else negative = 1.0D;
                ...
                double range = 2.0D;
                List<EntityBabylonWeaponSS> weapons = player.worldObj.getEntitiesWithinAABB(
            ...
            player.worldObj
                    .playSoundAtEntity(weapon, "botania:babylonSpawn", 1F, 1F + player.worldObj.rand.nextFloat() * 3F);
        }
    }
    ...
    public void onUsingTick(ItemStack stack, EntityPlayer player, int count) {
        ...
        if (count != stack.getMaxItemUseDuration() & count % 2 == 0 & !player.worldObj.isRemote) {
            ...
            if (WandManager.consumeVisFromInventory(
                    player,
                    new AspectList().add(Aspect.EARTH, this.TerraCost).add(Aspect.FIRE, this.IgnisCost)
                            .add(Aspect.ORDER, this.OrdoCost).add(Aspect.ENTROPY, this.PerditioCost))) {
