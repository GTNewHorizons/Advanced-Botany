...
public class ItemTeleportationTome extends Item implements IWarpingGear, IRepairable {
    ...
    public ItemStack onItemRightClick(ItemStack stack, World world, EntityPlayer player) {
        ...
        if (ItemNBTHelper.getInt(stack, "ICooldown", 0) != 0) return stack;

        if (true) {

            Entity pointedEntity = EntityUtils.getPointedEntity(world, player, 0.0D, 128.0D, 4F);

            if (player.isSneaking() & WandManager.consumeVisFromInventory(
                    player,
                ...
                SuperpositionHandler
                        .imposeBurst(world, player.dimension, player.posX, player.posY + 1, player.posZ, 1.25f);
                ...
                if (!world.isRemote) player.setPositionAndUpdate(lookVec.x, lookVec.y, lookVec.z);
                ...
                if (!world.isRemote) Main.packetInstance.sendToAllAround(
                        new PortalTraceMessage(
                                ...
                                finalVec.y,
                                finalVec.z,
                                player.getDistance(primalVec.x, primalVec.y, primalVec.z)),
                        new TargetPoint(player.dimension, player.posX, player.posY, player.posZ, 128.0D));

                ItemNBTHelper.setInt(stack, "ICooldown", 20);

                return stack;

            } else if (pointedEntity != null & pointedEntity instanceof EntityLivingBase
                    & WandManager.consumeVisFromInventory(
                            player,
                            new AspectList().add(Aspect.AIR, this.AerCost).add(Aspect.ORDER, this.OrdoCost)
                                    .add(Aspect.ENTROPY, this.PerditioCost))) {
                                        ...
                                        if (!world.isRemote) SuperpositionHandler.imposeBurst(
                                                world,
                                                player.dimension,
                                                player.posX,
                                                player.posY + 1,
                                        ...
                                        ((EntityLivingBase) pointedEntity)
                                                .setPositionAndUpdate(primalVec.x, primalVec.y, primalVec.z);
                                        ...
                                        MovingObjectPosition pointed = SuperpositionHandler
                                                .getPointedBlock(player, world, 128.0F);

                                        if (pointed != null & WandManager.consumeVisFromInventory(
                                                player,
                                                new AspectList().add(Aspect.AIR, this.AerCost)
                                                        .add(Aspect.ORDER, this.OrdoCost)
                                                        .add(Aspect.ENTROPY, this.PerditioCost))) {
                                            ...
                                            for (int counter = 0; counter <= 32; counter++) {
                                                ...
                                                if (!world.isAirBlock(x, y + counter - 1, z)
                                                        & world.getBlock(x, y + counter - 1, z).isCollidable()
