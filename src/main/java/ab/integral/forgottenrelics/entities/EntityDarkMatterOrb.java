...
public class EntityDarkMatterOrb extends EntityThrowable {
    ...
    public void onUpdate() {
        ...
        if (this.worldObj.isRemote) Thaumcraft.proxy.wispFXEG(
                this.worldObj,
                this.posX,
                (double) (float) (this.posY + 0.22 * this.height),
                this.posZ,
                (Entity) this);
    }
            ...
            for (int i = 0; i < list.size(); ++i) {
                ...
                if (entity1 instanceof EntityLivingBase) {
                    ...
                    if (this.worldObj.provider.dimensionId == Config.dimensionOuterId) {
                        ...
                        ((EntityLivingBase) entity1).attackEntityFrom(
                                new DamageRegistryHandler.DamageSourceDarkMatter((Entity) this.getThrower()),
                                RelicsConfigHandler.eldritchSpellDamageEx);
                        try {
                        ...
                        ((EntityLivingBase) entity1).attackEntityFrom(
                                new DamageRegistryHandler.DamageSourceDarkMatter((Entity) this.getThrower()),
                                RelicsConfigHandler.eldritchSpellDamage);
                        try {
                            ((EntityLivingBase) entity1).addPotionEffect(new PotionEffect(Potion.weakness.id, 160, 1));
                            ((EntityLivingBase) entity1)
                                    .addPotionEffect(new PotionEffect(Potion.moveSlowdown.id, 100, 1));
