...
public class EntityShinyEnergy extends EntityThrowable implements IEntityAdditionalSpawnData {
    ...
    public EntityShinyEnergy(final World par1World, final EntityLivingBase par2EntityLiving, EntityLivingBase t,
    ...
    public void onUpdate() {
        ...
        for (int i = 0; i < 8; i++) {
            Botania.proxy.sparkleFX(
                    worldObj,
                    this.posX + (Math.random() - 0.5) * 0.1,
                    this.posY + (Math.random() - 0.5) * 0.1,
                    this.posZ + (Math.random() - 0.5) * 0.1,
                    (float) (0.9F + (Math.random() * 0.1F)),
                    (float) (0.2F + (Math.random() * 0.2F)),
                    0.0F,
                    size,
                    2);
        }
        ...
        if (!this.worldObj.isRemote) {
            ...
            List<EntityLivingBase> targetList = worldObj
                    .getEntitiesWithinAABB(EntityLivingBase.class, this.boundingBox.expand(0.1, 0.1, 0.1));
