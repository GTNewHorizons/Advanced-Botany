...
public class EntityBabylonWeaponSS extends EntityThrowableCopy {
    ...
    public void onUpdate() {
        ...
        if (this.ticksExisted <= 15) {
            ...
            if (!worldObj.isRemote) {
                AxisAlignedBB axis = AxisAlignedBB
                        .getBoundingBox(posX, posY, posZ, lastTickPosX, lastTickPosY, lastTickPosZ).expand(2, 2, 2);
                ...
                for (EntityLivingBase living : entities) {
                    ...
                    if (living.hurtTime == 0) {
                        if (player != null) living.attackEntityFrom(
                                DamageSource.causeIndirectMagicDamage(this, player),
                                RelicsConfigHandler.damageApotheosisDirect);
                        ...
                        if (proportion > 1.2D) proportion = 1.2D;
        ...
        if (liveTime > delay) Botania.proxy.wispFX(worldObj, posX, posY, posZ, 1F, 1F, 0F, 0.3F, 0F);

        if (liveTime > 200 + delay) setDead();
    }

    public void invokeDamageEffects() {
        List<EntityLivingBase> targets = this.worldObj
                .getEntitiesWithinAABB(EntityLivingBase.class, this.boundingBox.expand(3.0, 3.0, 3.0));

        if (targets.contains(this.getThrower())) targets.remove(this.getThrower());

        for (int counterS = targets.size() - 1; counterS >= 0; counterS--) {
            if (!targets.get(counterS).isDead & !this.worldObj.isRemote) {
                targets.get(counterS).attackEntityFrom(
                        DamageSource.causeIndirectMagicDamage(this, this.getThrower()),
                        RelicsConfigHandler.damageApotheosisImpact);
    ...
    protected void onImpact(MovingObjectPosition pos) {
        ...
        if (pos.entityHit == null || pos.entityHit != thrower) {
            ...
            if (!this.worldObj.isRemote) Main.packetInstance.sendToAllAround(
                    new ApotheosisParticleMessage(this.posX, this.posY, this.posZ, 40),
