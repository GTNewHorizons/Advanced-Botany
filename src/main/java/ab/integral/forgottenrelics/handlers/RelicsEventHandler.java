...
public class RelicsEventHandler {
    ...
    public void onEntityAttacked(LivingAttackEvent event) {
        ...
        if (event.source.getEntity() instanceof EntityPlayer & !event.isCanceled()) {
            ...
            if (attackerPlayer.inventory.hasItem(Main.itemChaosCore) & Math.random() < 0.45) {
                List<Entity> entityList = event.entity.worldObj.getEntitiesWithinAABBExcludingEntity(
                        event.entity,
                        AxisAlignedBB.getBoundingBox(
                                event.entity.posX - 16,
                                event.entity.posY - 16,
                                event.entity.posZ - 16,
                                event.entity.posX + 16,
                                event.entity.posY + 16,
                                event.entity.posZ + 16));
        ...
        if (event.entity instanceof EntityPlayer) {
            ...
            /*
             * Handler for randomly teleport player who has Nebulous Core equipped, instead of taking damage from
             * attack.
             */

            if (!event.isCanceled() & SuperpositionHandler.hasBauble(player, Main.itemArcanum)
                    & Math.random() < RelicsConfigHandler.nebulousCoreDodgeChance
                    & !SuperpositionHandler.isDamageTypeAbsolute(event.source)) {
            ...
            if (!event.isCanceled() & SuperpositionHandler.hasBauble(player, Main.itemDarkSunRing)
                    & Main.darkRingDamageNegations.contains(event.source.damageType)) {
            ...
            else if (!event.isCanceled() & SuperpositionHandler.hasBauble(player, Main.itemDarkSunRing)
                    & event.source.getEntity() != null
                    & Math.random() <= RelicsConfigHandler.darkSunRingDeflectChance) {
    ...
    public void onEntityHurt(LivingHurtEvent event) {
        ...
        if (event.entity instanceof EntityPlayer & !event.isCanceled()) {
            ...
            if (player.inventory.hasItem(Main.itemFalseJustice)
                    & !SuperpositionHandler.isDamageTypeAbsolute(event.source)) {
                event.setCanceled(true);
                if (event.source.getEntity() == null) {
                    ...
                    player.attackEntityFrom(
                            new DamageRegistryHandler.DamageSourceTrueDamage(event.source.getEntity()),
                            event.ammount * 2);
                }
            }
         ...
         */
            ...
            if (attackerPlayer.inventory.hasItem(Main.itemFalseJustice)
                    & !SuperpositionHandler.isDamageTypeAbsolute(event.source)) {
                event.setCanceled(true);
        ...
        if (event.entity instanceof EntityPlayer & !event.isCanceled()) {
            ...
            /*
             * Handler for multiplication of damage dealt to owners of Chaos Core by value in range between 0.0 and 2.0.
             */
            ...
            if (!event.isCanceled() & event.ammount > 100.0F
                    & SuperpositionHandler.hasBauble(player, Main.itemDarkSunRing)) {
                event.setCanceled(true);
            }
            ...
            if (SuperpositionHandler.hasBauble(player, Main.itemDarkSunRing) & !event.isCanceled()
                    & Math.random() <= 0.25
                    & !SuperpositionHandler.isDamageTypeAbsolute(event.source)) {
            ...
            /*
             * Handler for redirecting damage received by player to owner of Ancient Aegis nearby, if one is present.
             */

            if (!event.entity.worldObj.isRemote & !SuperpositionHandler.hasBauble(player, Main.itemAncientAegis)
                    & !event.isCanceled()) {
                EntityPlayer aegisOwner = SuperpositionHandler
                        .findPlayerWithBauble(event.entity.worldObj, 32, Main.itemAncientAegis, player);
            ...
            if (SuperpositionHandler.hasBauble(player, Main.itemAncientAegis) & !event.isCanceled()
                    & !SuperpositionHandler.isDamageTypeAbsolute(event.source)) {
                event.ammount *= 1.0F - RelicsConfigHandler.ancientAegisDamageReduction;
            }

            /*
             * Handler for splitting damage dealt to wearer of Superposition Ring among all other wearers, if any exist.
             */

            if (SuperpositionHandler.hasBauble(player, Main.itemSuperpositionRing) & !event.isCanceled()) {

                List superpositioned = SuperpositionHandler
                        .getBaubleOwnersList(player.worldObj, Main.itemSuperpositionRing);
                if (superpositioned.contains(player)) superpositioned.remove(player);
            ...
            if (!event.isCanceled() & SuperpositionHandler.hasBauble(player, Main.itemOblivionAmulet)
                    & !SuperpositionHandler.isDamageTypeAbsolute(event.source)) {
                if (WandManager.consumeVisFromInventory(
                        player,
                        new AspectList()
                                .add(Aspect.FIRE, (int) (event.ammount * 8 * RelicsConfigHandler.oblivionAmuletVisMult))
                                .add(
                                        Aspect.ENTROPY,
                                        (int) (event.ammount * 8 * RelicsConfigHandler.oblivionAmuletVisMult)))) {
                    ...
                    ItemNBTHelper.setFloat(
                            oblivionAmulet,
                            "IDamageStored",
                            ItemNBTHelper.getFloat(oblivionAmulet, "IDamageStored", 0) + event.ammount);
    ...
    public void onDeath(LivingDeathEvent event) {
        ...
        if (event.entity instanceof EntityPlayer
                & !(event.source instanceof DamageRegistryHandler.DamageSourceTrueDamage)
                & !(event.source instanceof DamageRegistryHandler.DamageSourceTrueDamageUndef)) {
                    ...
                    if (fateTomeStack.hasTagCompound()) {
                        ...
                        if (ItemNBTHelper.verifyExistance(fateTomeStack, "IFateCooldown"))
                            if (ItemNBTHelper.getInt(fateTomeStack, "IFateCooldown", 0) == 0)
                                if (WandManager.consumeVisFromInventory(
                                        player,
                                        new AspectList().add(Aspect.AIR, ItemFateTome.AerCost)
                                                .add(Aspect.EARTH, ItemFateTome.TerraCost)
                                                .add(Aspect.FIRE, ItemFateTome.IgnisCost)
                                                .add(Aspect.WATER, ItemFateTome.AquaCost)
                                                .add(Aspect.ORDER, ItemFateTome.OrdoCost)
                                                .add(Aspect.ENTROPY, ItemFateTome.PerditioCost))) {

                                                    ItemNBTHelper.setInt(
                                                            fateTomeStack,
                                                            "IFateCooldown",
                                                            (int) (600 + Math.random() * 1200));
                                                    event.setCanceled(true);
                                                    ...
                                                    SuperpositionHandler.imposeBurst(
                                                            player.worldObj,
                                                            player.dimension,
                                                            player.posX,
                                                            player.posY + 1,
