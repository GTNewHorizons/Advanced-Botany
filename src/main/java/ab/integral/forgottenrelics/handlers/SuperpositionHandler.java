...
public class SuperpositionHandler {
    ...
    public static void imposeArcLightning(World world, int dimension, double x, double y, double z, double destX,
            double destY, double destZ, float r, float g, float b, float h) {
        if (!world.isRemote) Main.packetInstance.sendToAllAround(
                new ArcLightningMessage(x, y, z, destX, destY, destZ, r, g, b, h),
                new TargetPoint(dimension, x, y, z, 128.0D));
    }

    public static void imposeLightning(World world, int dimension, double x, double y, double z, double destX,
            double destY, double destZ, int duration, float curve, int speed, int type, float width) {
        if (!world.isRemote) Main.packetInstance.sendToAllAround(
    ...
    public static void imposeBurst(World world, int dimension, double x, double y, double z, float size) {
        if (!world.isRemote) Main.packetInstance
                .sendToAllAround(new BurstMessage(x, y, z, size), new TargetPoint(dimension, x, y, z, 128.0D));
    }

    /**
     * Registers aspect tag on given item, for all it's existing meta-ID variants, within range specified by startCount
     * and metaLimit.
     */
    ...
    public static boolean isDamageTypeAbsolute(DamageSource source) {
        if (source == DamageSource.outOfWorld || source == DamageSource.starve
                || source instanceof DamageRegistryHandler.DamageSourceFate
                || source instanceof DamageRegistryHandler.DamageSourceOblivion
                || source instanceof DamageRegistryHandler.DamageSourceSoulDrain
                || source instanceof DamageRegistryHandler.DamageSourceTrueDamage
    ...
    public static boolean isEntityBlacklistedFromTelekinesis(EntityLivingBase entity) {
        if (entity instanceof EntityThaumcraftBoss || entity instanceof EntityDoppleganger) return true;
        else return false;
    }
    ...
    public static List<EntityPlayer> getBaubleOwnersList(World world, Item baubleItem) {
        ...
        if (!world.isRemote) {
            ...
            List<EntityPlayer> playersList = new ArrayList(
    ...
    /**
     * Searches for players within given radius from given entity that have specified item within their bauble
     * inventory, and constructs a list of them.
     * 
    ...
    public static EntityPlayer findPlayerWithBauble(World world, int radius, Item baubleItem, EntityLivingBase entity) {
        ...
        if (!world.isRemote) {
            List<EntityPlayer> playerList = world.getEntitiesWithinAABB(
                    EntityPlayer.class,
            ...
            if (playerList.contains(entity)) playerList.remove(entity);
    ...
    public static boolean hasBauble(EntityPlayer player, Item theBauble) {
        ...
        if (baubles.getStackInSlot(0) != null) baubleList.add(baubles.getStackInSlot(0).getItem());
        if (baubles.getStackInSlot(1) != null) baubleList.add(baubles.getStackInSlot(1).getItem());
        if (baubles.getStackInSlot(2) != null) baubleList.add(baubles.getStackInSlot(2).getItem());
        if (baubles.getStackInSlot(3) != null) baubleList.add(baubles.getStackInSlot(3).getItem());
    ...
    public static boolean validTeleport(Entity entity, double x_init, double y_init, double z_init, World world) {
        ...
        if (block != Blocks.air & block.isCollidable()) {
            ...
            for (int counter = 0; counter <= 32; counter++) {
                ...
                if (!world.isAirBlock(x, y + counter - 1, z) & world.getBlock(x, y + counter - 1, z).isCollidable()
                        & world.isAirBlock(x, y + counter, z)
                        & world.isAirBlock(x, y + counter + 1, z)) {
                    ...
                    entity.worldObj
                            .playSoundEffect(entity.posX, entity.posY, entity.posZ, "mob.endermen.portal", 1.0F, 1.0F);
    ...
    public static ItemStack getRandomValidWand(List<ItemStack> list, Aspect aspect) {
        ...
        if (list.size() > 0) {
            ...
            for (int counter = 0; counter < list.size(); counter++) {
                ...
                if (((ItemWandCasting) sheduledWand.getItem()).getVis(sheduledWand, aspect)
                        < ((ItemWandCasting) sheduledWand.getItem()).getMaxVis(sheduledWand)) {
    ...
    public static boolean sidedVisConsumption(EntityPlayer player, World world, AspectList list) {
        ...
        if (!world.isRemote) {
            if (WandManager.consumeVisFromInventory(player, list)) return true;
            else return false;
        } else {
            return true;
        }
    ...
    public static void insanelyDisastrousConsequences(EntityPlayer player) {
        ...
        List<EntityLivingBase> entityList = player.worldObj.getEntitiesWithinAABB(
        ...
        if (!(entityList == null) & entityList.size() > 0) {
            ...
            for (int counter = entityList.size(); counter > 0; counter--) {
                ...
                player.worldObj.newExplosion(
                        player,
                        entityList.get(counter - 1).posX,
                        entityList.get(counter - 1).posY,
                        entityList.get(counter - 1).posZ,
                        16F,
                        true,
                        true);
            }
