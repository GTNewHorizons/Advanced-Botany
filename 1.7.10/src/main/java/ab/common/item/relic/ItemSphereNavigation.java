...
public class ItemSphereNavigation extends ItemModRelic {
	...
	public void registerIcons(IIconRegister ir) {
		...
		for(int i = 0; i < icons.length; i++)
			icons[i] = icons[i] = ir.registerIcon("ab:" + "sphereNavigation_" + i);
	}
	...
	public String getItemStackDisplayName(ItemStack stack) {
	    ...
	    return super.getItemStackDisplayName(stack) + ((rStack == null || rStack.getItem() == null) ? "" : (EnumChatFormatting.RESET + " (" + EnumChatFormatting.GREEN + rStack.getDisplayName() + EnumChatFormatting.RESET + ")"));
	}
		...
		if(world.isRemote) {
			...
			for(int y = -rangeSearch * 2; y < rangeSearch; y++) {	
				for(int x = -rangeSearch; x < rangeSearch; x++) {
					if(!(world.rand.nextInt(maxFindedBlocks) < (maxFindedBlocks - findedBlocks)) || world.rand.nextBoolean())
						continue;
			...
			if(renderStack != null)
				ItemsRemainingRender.set(renderStack, StatCollector.translateToLocal("ab.sphereNavigation.founded") + " " + findedBlocks);
		}
	}
