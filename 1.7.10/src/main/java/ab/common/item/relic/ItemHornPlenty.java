			...
			if(!player.worldObj.isRemote && horn != null && hasChargeLoot(horn)) {
				if(player.worldObj.rand.nextInt(100) < 20 && event.entityLiving != null && !(event.entityLiving instanceof IBossDisplayData) && isVallidEntity(event.entityLiving)) {
					try {
						...
						ReflectionHelper.findMethod(EntityLivingBase.class, liv, dropFewItems,  new Class[] { boolean.class, int.class } ).invoke(liv, new Object[] { true, (int)(event.lootingLevel * 1.5f) });
