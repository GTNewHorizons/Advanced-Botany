...
public class ItemBlackHalo extends ItemMod implements IBlockProvider {
	...
	public ItemStack onItemRightClick(ItemStack halo, World world, EntityPlayer player) {
		...
		for(int i = 0; i < 9; i++) {
			...
			if(stack != null && stack.getItem() instanceof ItemBlackHoleTalisman) {
				boolean hasBlock = !Block.isEqualTo(((ItemBlackHoleTalisman)stack.getItem()).getBlock(stack), Blocks.air) && ((ItemBlackHoleTalisman)stack.getItem()).getBlock(stack) != null;
				if(hasBlock) {
					if(this.getItemForSlot(halo, segment) != null)
						return halo;
					this.setItemSlot(halo, stack, segment);
	...
	public boolean onItemUse(ItemStack halo, EntityPlayer player, World world, int par4, int par5, int par6, int par7, float par8, float par9, float par10) {
		for(int i = 0; i < 12; i++) {
			...
			if (tile != null && tile instanceof IInventory) {
				...
				for (int slot : slots) {
					...
					if (stackInSlot == null) {
						...
						if (stack.stackSize != 0 && inv.isItemValidForSlot(slot, stack) && (!(inv instanceof ISidedInventory) || ((ISidedInventory)inv).canInsertItem(slot, stack, par7))) {
	...
	public static void renderHUD(ScaledResolution resolution, EntityPlayer player, ItemStack halo) {
	    ...
	    if(stack != null) {
	    	...
	    	GL11.glEnable(32826);
	    	RenderItem.getInstance().renderItemAndEffectIntoGUI(mc.fontRenderer, mc.renderEngine, stack, resolution.getScaledWidth() / 2 - 8, resolution.getScaledHeight() / 2 - 52);
	...
	public void render(ItemStack stack, EntityPlayer player, float partialTicks) {
	    ...
	    for (int seg = 0; seg < 12; seg++) {
	      ...
	      if (slotStack != null) {
	          ...
	          RenderBlocks.getInstance().renderBlockAsItem(talisman.getBlock(slotStack), talisman.getBlockMeta(slotStack), 1.0F);
	      } 
	      GL11.glPopMatrix();
	      GL11.glPushMatrix();
	...
	public int getBlockCount(EntityPlayer player, ItemStack requestor, ItemStack halo, Block block, int meta) {
		for(int i = 0; i < 12; i++) {
		    ...
		    int storedMeta = talisman.getBlockMeta(tal);
		    if (stored == block && storedMeta == meta)
		    	return talisman.getBlockCount(tal);
		} 
	    return 0;
	  }
}