...
public class ItemNebulaArmor extends ItemManasteelArmor implements IManaItem, IManaTooltipDisplay {
	...
	public ISpecialArmor.ArmorProperties getProperties(EntityLivingBase player, ItemStack armor, DamageSource source, double damage, int slot) {
		return new ISpecialArmor.ArmorProperties(0, getArmorMaterial().getDamageReductionAmount(slot) * (0.0175f + (0.0525D * (1.0f - (this.getDamage(armor) / 1000.0f)))), 2147483647);
	}
	...
	public boolean hasArmorSetItem(EntityPlayer player, int i) {
	    ...
	    switch (i) {
	    	case 0:
	    		return (stack.getItem() == ItemListAB.itemNebulaHelm || stack.getItem() == ItemListAB.itemNebulaHelmReveal);
	    	case 1:
