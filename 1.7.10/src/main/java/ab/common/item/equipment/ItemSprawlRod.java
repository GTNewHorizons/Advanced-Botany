...
public class ItemSprawlRod extends ItemMod implements IManaUsingItem {
	...
	public void onUpdate(ItemStack stack, World world, Entity player, int par4, boolean par5) {
		if(!world.isRemote && player instanceof EntityPlayer && stack.getItemDamage() > 0 && ManaItemHandler.requestManaExactForTool(stack, (EntityPlayer)player, 760, true))
	    	stack.setItemDamage(stack.getItemDamage() - 1); 
	}
	
	public ItemStack onItemRightClick(ItemStack rod, World world, EntityPlayer p) {
		if(rod.getItemDamage() != 0)
			return rod;
		ItemStack seed = null;
	...
	public void onUsingTick(ItemStack stack, EntityPlayer p, int time) {
		if(p.worldObj.isRemote) {
			...
			Botania.proxy.wispFX(p.worldObj, posX, posY, posZ, color.getRed() / 255.0f, color.getGreen() / 255.0f, color.getBlue() / 255.0f, 0.5F * fTicks - (float)(Math.random() * 0.1f), 0.0f, 0.5F);
		}
	}
