...
public class ItemNebulaBoots extends ItemNebulaArmor {
	...
	public void onArmorTick(World world, EntityPlayer player, ItemStack stack) {
		...
		if(world.isRemote && player.getCurrentArmor(0) != null && player.getCurrentArmor(0).getItem() instanceof ItemNebulaBoots && player.isSprinting()) {
			float w = 0.6F;
	        float c = 1.0F - w;
	        ...
	        for(int i = 0; i < 2; i++)
	        	Botania.proxy.sparkleFX(world, player.posX + (Math.random() - 0.5f), player.posY - 1.25f + (Math.random() / 4 - 0.125f), player.posZ + (Math.random() - 0.5f), r, g, b, (float)(0.7f + (Math.random() / 2)), 25);
		}
	}
