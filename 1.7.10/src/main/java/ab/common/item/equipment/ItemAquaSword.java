		...
		if(!player.worldObj.isRemote) {
			...
			if(ManaItemHandler.requestManaExactForTool(stack, player, 10, false)) {
				for(int i = 0; i < 24; i++) {
					...
					Botania.proxy.wispFX(player.worldObj, posX, posY, posZ, 0.0f, (float)(Math.random() * 0.35f), 1.0f - (float)(Math.random() * 0.4f), 0.17f + (float)(Math.random() * 0.3f), mtX, mtY, mtZ, 0.512f);
				}
			}
		}
	...
	public void onUsingTick(ItemStack stack, EntityPlayer p, int time) {
		if(p.worldObj.isRemote) {
			...
			for(int i = 0; i < wispCount; i++) {
				...
				Botania.proxy.wispFX(p.worldObj, posX, posY, posZ, 0.0f, (float)(Math.random() * 0.35f), 1.0f - (float)(Math.random() * 0.4f), 0.3F, 0.0f, -0.1f + (float)(Math.random() * 0.05f), 0.0f, 0.7F);
				wticks += tickIncrement;
			}
		} else {
			AxisAlignedBB axis = AxisAlignedBB.getBoundingBox(p.posX, p.posY, p.posZ, p.lastTickPosX, p.lastTickPosY, p.lastTickPosZ).expand(2.75f, 2.75f, 2.75f);
			List<EntityLivingBase> entities = p.worldObj.getEntitiesWithinAABB(EntityLivingBase.class, axis);
			for(EntityLivingBase living : entities) {
				if(living instanceof EntityPlayer && (((EntityPlayer)living).getCommandSenderName().equals(p.getCommandSenderName()) || (MinecraftServer.getServer() != null && !MinecraftServer.getServer().isPVPEnabled())))
					continue; 
