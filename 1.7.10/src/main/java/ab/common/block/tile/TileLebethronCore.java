...
public class TileLebethronCore extends TileMod implements IRenderHud {
	...
	public Block getBlock() {
		if(!Block.isEqualTo(block, Blocks.air) && block != null)
			return this.block;
		return null;
	}
	...
	public boolean hasValidTree() {
		if(!checkBlock(this.worldObj.getBlock(this.xCoord, this.yCoord - 1, this.zCoord), this.worldObj.getBlockMetadata(this.xCoord, this.yCoord - 1, this.zCoord))) {
			return false;
		}
		for(int i = 1; i <= 4; i++) {
			if(!checkBlock(this.worldObj.getBlock(this.xCoord, this.yCoord + i, this.zCoord), this.worldObj.getBlockMetadata(this.xCoord, this.yCoord + i, this.zCoord))) {
				return false;
			}
		}
		return true;
	...
	private void spawnLeaves() {
		...
		this.setBlock(worldObj, this.xCoord, this.yCoord + 7, this.zCoord - 1);	
		for(int i = 0; i <= 3; i++)
			this.setBlock(worldObj, this.xCoord, this.yCoord + 6 + i, this.zCoord);	
