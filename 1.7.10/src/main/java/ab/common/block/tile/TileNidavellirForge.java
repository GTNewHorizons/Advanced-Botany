...
public class TileNidavellirForge extends TileInventory implements ISparkAttachable, ISidedInventory {
	...
	private void updateClient() {
		if(this.mana > 0) {
			...
			for(int i = 0; i < totalSpiritCount; i++) {
				...
				Botania.proxy.wispFX(this.worldObj, x, y, z, colorsfx[0], colorsfx[1], colorsfx[2], 0.85F * size, (float)g * 0.05F, 0.25F);
				Botania.proxy.wispFX(this.worldObj, x, y, z, colorsfx[0], colorsfx[1], colorsfx[2], (float)Math.random() * 0.1F + 0.1F * size, (float)(Math.random() - 0.5D) * 0.05F, (float)(Math.random() - 0.5D) * 0.05F, (float)(Math.random() - 0.5D) * 0.05F, 0.9F);
				if(ticks == 100)
					for(int j = 0; j < 12; j++)
						Botania.proxy.wispFX(this.worldObj, this.xCoord + 0.5D, this.yCoord + 1.1D - indetY, this.zCoord + 0.5D, colorsfx[0], colorsfx[1], colorsfx[2], (float)Math.random() * 0.15F + 0.15F * size, (float)(Math.random() - 0.5D) * 0.125F * size, (float)(Math.random() - 0.5D) * 0.125F * size, (float)(Math.random() - 0.5D) * 0.125F * size, 0.8f);  
			}
		}
	}
	...
	public boolean canInsertItem(int slot, ItemStack stack, int side) {
		for(int i = 1; i < getSizeInventory(); i++) {
			...
			if(slotStack != null && slotStack.stackSize == slotStack.getMaxStackSize() && isItemEqual(stack, slotStack))
				return false;
		}
