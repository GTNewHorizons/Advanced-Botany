...
public class TileMagicCraftCrate extends TileMagicWorkbench implements ISidedInventory, IInventory {
	...
	public TileMagicCraftCrate() {
		pattern = new boolean[][] { new boolean[] { true, true, true }, new boolean[] { true, true, true }, new boolean[] { true, true, true } };
	}

	public void updateEntity() {
		if(!worldObj.isRemote && (craft(true) && canEject() || isFull() && !isWaiting()))
			ejectAll();
		int newSignal = 0;
			...
			if(aspects.size() > 0 && this.getStackInSlot(10) != null) {
				...
				if (wand.consumeAllVisCrafting(this.getStackInSlot(10), player, aspects, true)) {
					for(int var2 = 0; var2 < 9; ++var2) {
						...
						if(var3 != null)
							craft.decrStackSize(var2, 1);
					}
					for(int i = 0; i < 9; i++)
						setInventorySlotContents(i, craft.getStackInSlot(i));
	...
	public void readCustomNBT(NBTTagCompound nbtt) {
		...
		for(int i = 0; i < 3; i++)
			for(int j = 0; j < 3; j++)
				pattern[i][j] = nbtt.getBoolean(PATTERN_TAG + i * 3 + j);
		NBTTagCompound nbtt1 = nbtt.getCompoundTag(WAITING_TAG);
		if(nbtt1 != null)
			waitingStack = ItemStack.loadItemStackFromNBT(nbtt1);
	}
	...
	public void writeCustomNBT(NBTTagCompound nbtt) {
		...
		for(int i = 0; i < 3; i++)
			for(int j = 0; j < 3; j++)
				nbtt.setBoolean(PATTERN_TAG + i * 3 + j, pattern[i][j]);
