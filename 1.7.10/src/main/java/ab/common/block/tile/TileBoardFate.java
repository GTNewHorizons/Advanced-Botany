...
public class TileBoardFate extends TileInventory {
	...
	public boolean spawnRelic(EntityPlayer player) {
		...
		if(relicCount < 1)
			...
			if(hasRelicAchievement(player, relic) || !ConfigABHandler.fateBoardRelicEnables[relicCount - 1]) {
				...
				player.addChatMessage((new ChatComponentTranslation("botaniamisc.diceRoll", new Object[] { Integer.valueOf(relicCount) })).setChatStyle((new ChatStyle()).setColor(EnumChatFormatting.DARK_GREEN)));
	...
	private void dropRelic(EntityPlayer player, int slot) {
		EntityItem entityItem = new EntityItem(worldObj, xCoord + 0.5f, yCoord + 0.8f, zCoord + 0.5f, getStackInSlot(slot).copy());
		float f3 = 0.15F;
