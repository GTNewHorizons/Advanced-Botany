	...
	public void onUpdate() {
		...
		if(this.ticksExisted % 10 == 0 && this.mana >= manaRequare) {
			label666: for(EntityItem item : items) {
				if(item.isDead || !(item.getEntityItem().stackSize >= 1))
					continue;
				...
				if(item.getEntityItem().getItem() instanceof ItemLexicon) {
					if(!((ItemLexicon)item.getEntityItem().getItem()).isKnowledgeUnlocked(item.getEntityItem(), RecipeListAB.forgotten)) {				
						if(world.isRemote)
							spawnParticle(world, item);
						else {
							if(item.getEntityItem().stackSize > 1)
		...
		if(world.isRemote)
			for (int i = 0; i < 10; i++) {
				...
				if(stack.getItem() instanceof net.minecraft.item.ItemBlock) {
					world.spawnParticle("blockcrack_" + Item.getIdFromItem(stack.getItem()) + "_" + stack.getItemDamage(), item.posX, item.posY, item.posZ, mx, my, mz);
				} else {
