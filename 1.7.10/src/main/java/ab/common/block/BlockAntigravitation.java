...
public class BlockAntigravitation extends Block {
	...
	public void randomDisplayTick(World world, int x, int y, int z, Random random) {	
		if(random.nextBoolean()) {
			...
			Botania.proxy.wispFX(world, x + 0.1f + Math.random() * 0.8f, y - 0.18f + 1.0f, z + 0.1 + Math.random() * 0.8, r, g, b, 0.24F * (float)(Math.random() - 0.5D), 0.0f, (float)(0.001f + Math.random() * 0.01f), 0.0f);
		}
	}
}