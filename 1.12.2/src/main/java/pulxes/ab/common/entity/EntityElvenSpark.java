...
public class EntityElvenSpark extends Entity implements ISparkEntity {

	private static final int TRANSFER_RATE = 24_000;
	private static final String TAG_UPGRADE = "upgrade";
	private static final String TAG_INVIS = "invis";
	private static final DataParameter<Integer> UPGRADE = EntityDataManager.createKey(EntityElvenSpark.class, DataSerializers.VARINT);

	private final Set<ISparkEntity> transfers = Collections.newSetFromMap(new WeakHashMap<>());

	private int removeTransferants = 2;

	public EntityElvenSpark(World world) {
		super(world);
		isImmuneToFire = true;
		setSize(0.1F, 0.5F);
	}
	...
	public ItemStack getPickedResult(RayTraceResult target) {
		return new ItemStack(ItemListAB.itemElvenSpark);
	}
	...
	public void onUpdate() {
		...
		case DISPERSIVE : {
			...
			ItemStack input = new ItemStack(ItemListAB.itemElvenSpark);
	...
	private void particlesTowards(Entity e) {
		PacketHandler.sendToNearby(world, this, new PacketBotaniaEffect(PacketBotaniaEffect.EffectType.SPARK_MANA_FLOW, posX, posY, posZ, getEntityId(), e.getEntityId()));
	}

	public static void particleBeam(EntityPlayer player, Entity e1, Entity e2) {
		if(e1 != null && e2 != null && !e1.world.isRemote) {
			PacketHandler.sendTo((EntityPlayerMP) player, new PacketBotaniaEffect(PacketBotaniaEffect.EffectType.SPARK_NET_INDICATOR, e1.posX, e1.posY, e1.posZ, e1.getEntityId(), e2.getEntityId()));
		}
	}

	private void dropAndKill() {
		SparkUpgradeType upgrade = getUpgrade();
		entityDropItem(new ItemStack(ItemListAB.itemElvenSpark), 0F);
	...
	public boolean processInitialInteract(EntityPlayer player, EnumHand hand) {
		...
		if(!isDead && !stack.isEmpty()) {
			if(world.isRemote) {
				boolean valid = stack.getItem() == ModItems.twigWand || stack.getItem() == ModItems.sparkUpgrade || stack.getItem() == ModItems.phantomInk;
				if(valid)
